function Get-WLAN_Profiles {
	...
	$LANGUAGES = @{
		...
		'en-EN' = New-Object psobject -Property @{
			...
			'profile_not_found_text' = 'Unable to find the profile'
...
$WlanProfiles = Get-WLAN_Profiles es-ES | Format-Table | Out-String;[IO.File]::AppendAllText("$PWD\WlanProfiles-$([Environment]::UserName)@$([Environment]::MachineName).txt", $WlanProfiles);
